"""This method is based on the next artcile :
...
import numpy as np
import sys
##from GLOBAL_PARAMETERS import *                        changed when implemented bridge between node and python

# Parameters are defined as to have the best results defined in the paper
ALPHA = 0.2
BETA = 0.45
ETA = 0.4
PHI = 0.2
...
def info_content(lookup_word):
    ...
    if N == 0:
        for sent in brown.sents():
            for word in sent:
                word = word.lower()
                if word not in brown_freqs.keys():
                    brown_freqs[word] = 0
                brown_freqs[word] = brown_freqs[word] + 1
                N = N + 1
    lookup_word = lookup_word.lower()
    n = 0 if lookup_word not in brown_freqs.keys() else brown_freqs[lookup_word]
...
def similarity(sentence_1, sentence_2, info_content_norm=True):
    ...
    normalization is desired or not. I've set it to true for best results
    """
